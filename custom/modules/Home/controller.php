...
class HomeController extends SugarController
{
    ...
    function action_lookup()
    {
        ...
        $lookUpSql = "SELECT bc_storedata.name,
                ...
                WHERE (MATCH(name, description, catalognumber, sku, supplierid, immunogen, purchasingemail, purchasingname, supportemail, supportname) AGAINST ('{$searchString}*' IN BOOLEAN MODE)
                  OR MATCH(order_number_c, customer_po_number_c, order_status_c, other_notes_c, supplier_url_c, admin_immunogen_c) AGAINST ('{$searchString}*' IN BOOLEAN MODE))
                  AND deleted = 0";
