                    ...
                    if (in_array($module, array('Users', 'Employees'))) {
                        $fullName = from_html(
                                $locale->getLocaleFormattedName(
                                        $bean->first_name, $bean->last_name, '', $bean->title
                                )
                        );
                    } else if (SugarModule::get($module)->moduleImplements('Person')) {
                        $fullName = from_html(
                                $locale->getLocaleFormattedName(
                                        $bean->first_name, $bean->last_name, $bean->salutation, $bean->title
                                )
                        );
                    }
...
// fn
    ...
    function fill_in_additional_list_fields() {
        ...
        if ($row) {
            $this->attachment_image = SugarThemeRegistry::current()->getImage(
                    'attachment', '', null, null, '.gif', translate('LBL_ATTACHMENT', 'Emails')
            );
        } else {
    ...
    function _generateSearchImportWhereClause() {
        ...
        /**
         * Add the Option to search email by body
         * and add the new if condition to add % to both end of search string
         * Date - 16_04_12
         * @author Dhaval Darji
         */
        $availableSearchParam = array('name' => array('table_name' => 'emails'),
            ...
            //Dhaval
            'body' => array('table_name' => 'emails_text', 'db_key' => 'description', 'opp' => 'textSearch'),
            'status' => array('table_name' => 'emails', 'db_key' => 'status', 'opp' => '='),
                //END
        );
        ...
        foreach ($availableSearchParam as $key => $properties) {
            if (!empty($_REQUEST[$key])) {
                ...
                if ($searchValue == 'Not Closed' && $db_key == 'status') {
                    $opp = '<>';
                    $searchValue = 'Closed';
                }
                //END
