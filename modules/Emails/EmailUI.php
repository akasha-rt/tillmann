...
eoq;
    ...
    function _generateComposeConfigData($type = "email_compose_light") {
        ...
        /**
         * For Status dropdown search
         * @author Dhaval Darji 
         */
        $statusOptions = $app_list_strings['dom_email_status'];
        $statusOptions[] = '';
        $statusOptions['Not Closed'] = 'Not Closed';
        asort($statusOptions);
        $status = json_encode(get_select_options_with_id($statusOptions, 'unread'));
        $this->smarty->assign('status', $status);
        $this->smarty->assign('statusOptions', $statusOptions);
        //End - Dhaval
...
// fn
    ...
    /**
     * Retrieve Email GUID from ie_id for custom status
     * @author Dhaval Darji 
     */
    function getEmailGuId($overview) {
        global $db;
        $getToAddress = substr($overview->to, strpos($overview->to, '<') + 1, strpos($overview->to, '>') - strpos($overview->to, '<') - 1);
        $emailGuId = $db->fetchByAssoc(
                $db->query("SELECT
                                    id
                                FROM emails
                                WHERE mailbox_id = '{$overview->ie_id}'
                                    AND message_id = '{$overview->message_id}{$getToAddress}'
                                    AND deleted = 0"));
        return ($emailGuId['id']) ? $emailGuId['id'] : 1;
    }
                ...
                foreach ($overviews['retArr'] as $k => $overview) {
                    if (in_array($overview->uid, $exUids)) {
                        switch ($type) {
                            ...
                            case "Pending":
                                $email = new Email();
                                $email->retrieve($this->getEmailGuId($overview));
                                $email->status = 'Pending';
                                $email->save();
                                break;
                            case "Closed":
    ...
    function handleReplyType($email, $type) {
        ...
        switch ($type) {
            ...
            case "replyAll":
                ...
                /**
                 * Added extra case code to add ref. no if email's parent is set to case
                 * @author Dhaval Darji 
                 */
                if (isset($email->parent_type) && $email->parent_type == 'Cases') {
                    $myCase = new aCase();
