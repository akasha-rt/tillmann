...
class quicksearchQuery {
    ...
    function get_contact_array($args) {
        ...
        for ($i = 0; $i < count($list_return); $i++) {
            ...
            $contactName = $locale->getLocaleFormattedName($list_arr['fields'][$i]['first_name'], $list_arr['fields'][$i]['last_name'], $list_arr['fields'][$i]['salutation']);
    ...
    /**
     * To Enable Quick Search for Email Field Created new method
     * @author Dhaval darji
     */
    function getEmail($args) {
        ...
        foreach ($args['modules'] as $module) {
            ...
            $curlist = $focus->db->fetchByAssoc($focus->db->query("SELECT
                                              GROUP_CONCAT(email_address) AS list
                                            FROM email_addresses
                                              LEFT JOIN email_addr_bean_rel
                                                ON email_addresses.id = email_addr_bean_rel.email_address_id
                                            WHERE email_addresses.deleted = 0
                                                AND email_addr_bean_rel.deleted = 0
                                                AND email_addresses.email_address LIKE '{$_REQUEST['query']}%'
                                                AND email_addr_bean_rel.bean_module = '{$module}'
                                                ORDER BY email_addresses.email_address 
                                                LIMIT 10"));
            $curlist = explode(",", $curlist['list']);
            $list_return = array_merge($list_return, $curlist);
        }
        //Replacement for FormatResult function
        $list_arr['totalCount'] = count($list_return);
        foreach ($list_return as $key => $val) {
            $list_arr['fields'][$key]['module'] = $module;
            $list_arr['fields'][$key]['email1'] = $val;
        }
        return $json->encodeReal($list_arr);
    }
