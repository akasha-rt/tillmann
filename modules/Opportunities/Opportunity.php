...
class Opportunity extends SugarBean {
 ...
 AND contacts.deleted=0", "LEFT JOIN (SELECT
    ...
    /**
     * To update convertedtoopp to indicate if the case is converted to opportunity or not
     * @author Dhaval darji
     */
    function Updateconvertedtoopp($caseId) {
        $db = DBManagerFactory::getInstance();
        $query = "UPDATE cases
                    SET convertedtoopp = '1'
                    WHERE id = '{$caseId}' 
                    LIMIT 1";
        $result = $db->query($query, true, "Error Updating convertedtoopp for case : {$caseId}");
    }
