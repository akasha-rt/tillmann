...
if(/emailAddress\d+$/.test(targetEl.id)&&isValidEmail(targetEl.value)&&!this.verifying&&verifyElementFlag.value=="false"){
    ...
    this.verifying=true;
    var cObj=YAHOO.util.Connect.asyncRequest('GET','index.php?module=Contacts&action=RetrieveEmail&target='+targetEl.id+'&email='+targetEl.value,{
...
if(YAHOO.env.ua.ie>0){
    ...
    YAHOO.util.Event.addListener(newContent,"change",function(ev,el){
...
EmailAddressValidation:function(ev,fn,r,stR){
    YAHOO.util.Event.onContentReady(fn,function(){
